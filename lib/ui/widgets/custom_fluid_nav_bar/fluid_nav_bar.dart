...
class _FluidNavBarState extends State<FluidNavBar>
  ...
  Widget build(context) {
    ...
    final appSize = mediaQuery.size;
    final needBottomPadding = mediaQuery.viewPadding.bottom > 0;
    final height = FluidNavBar.nominalHeight;
    return Container(
      width: appSize.width,
      height: height + (needBottomPadding ? bottomPadding : 0),
      child: Stack(
        children: [
          Positioned(
            left: 0,
  ...
  List<FluidNavBarButton> _buildButtons() {
    List<MenuIconData> icons = [
      MenuIconData(
        icon: SvgPicture.asset(
          ICON_MENU_BOTTOM_BAR_NEWS,
          fit: BoxFit.cover,
          color: NewsThemeData.buttonMainColor.withOpacity(0.7),
        ).paddingAll(10),
      ...
      MenuIconData(
        icon: Icon(Icons.favorite_border,
            color: NewsThemeData.saveButtonColor.withOpacity(0.7)),
        activeIcon: Icon(Icons.favorite, color: NewsThemeData.saveButtonColor),
      ),
      MenuIconData(
        activeIcon: SvgPicture.asset(
          ICON_MENU_BOTTOM_BAR_RECOMMENDED,
          color: NewsThemeData.buttonMainColor,
        ).paddingAll(10),
        icon: SvgPicture.asset(
          ICON_MENU_BOTTOM_BAR_RECOMMENDED,
  ...
  void _handlePressed(int index) {
    ...
    Future.delayed(
      ...
      () {
        _yController.animateTo(1.0, duration: Duration(milliseconds: 1400));
      },
    );
...
class _BackgroundCurvePainter extends CustomPainter {
  ...
  static const _horizontalControlTop = 0.7;
  ...
  static const _bottomDistance = 16.0;
...
class MenuIconData {
  ...
  const MenuIconData({this.icon, this.activeIcon});
}
